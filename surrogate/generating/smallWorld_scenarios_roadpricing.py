...
from surrogate.visualizing.input_data import visualize_scenario_from_training_data
...
def generate_random_graph(args, ax):
...
def generate_random_population(args, G, ax):
...
def save_random_scenario(args, graph, population, ax):
    ...
    scenario["toll_links"] = get_toll_links(args, scenario, ax=ax)
    ...
    if args.verbose:
        visualize_scenario_from_training_data(axis=ax, training_data=scenario)
...
def get_toll_links(args, scenario, ax):
    center_x, center_y = np.mean(scenario["nodes_x"]), np.mean(scenario["nodes_y"])
    min_x, max_x, min_y, max_y = np.min(scenario["nodes_x"]), np.max(scenario["nodes_x"]), np.min(scenario["nodes_y"]), np.max(scenario["nodes_y"])
    distance_x = (max_x - min_x) * 0.1
    distance_y = (max_y - min_y) * 0.1
    min_x, max_x, min_y, max_y = center_x - distance_x, center_x + distance_x, center_y - distance_y, center_y + distance_y
    nodes_in_toll_area = []
    for node_idx, node_x, node_y in zip(scenario["nodes_id"], scenario["nodes_x"], scenario["nodes_y"]):
        if min_x < node_x < max_x and min_y < node_y < max_y:
            nodes_in_toll_area.append(node_idx)
    link_idxs_in_toll_area = []
    for link_idx, link_from, link_to in zip(scenario["links_id"], scenario["link_from"], scenario["link_to"]):
        if link_from in nodes_in_toll_area or link_to in nodes_in_toll_area:
            link_idxs_in_toll_area.append(link_idx)
    print(link_idxs_in_toll_area)
    return link_idxs_in_toll_area

def run(args):
    np.random.seed(args.seed)
    random.seed(args.seed)
    fig, ax = plt.subplots(figsize=(10, 10))
    graph = generate_random_graph(args, ax)
    population = generate_random_population(args, graph, ax)
    save_random_scenario(args, graph=graph, population=population, ax=ax)
