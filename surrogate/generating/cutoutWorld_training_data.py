...
def read_in_o_d_pairs(event_reader, relevant_links, scenario):
    ...
    events_in_area_of_interest = event_reader.events[["vehicle", "link", "time"]].merge(relevant_links[["link_id_OLD", "link_id"]], how="left", left_on="link", right_on="link_id_OLD")
    events_in_area_of_interest = events_in_area_of_interest.dropna(axis=0).drop(columns=["link_id_OLD"])
    vehicle_trips = events_in_area_of_interest.groupby('vehicle')[["link_id", "time"]].apply(lambda x: x.values.tolist()).to_dict()
...
def generate_training_data(args):
    ...
    # Read in scenario
    matsim_network, event_reader, trip_reader, _ = read_in_scenario(network_path=f"../matsim-berlin/scenarios/{args.simulation}/{get_scenario_acronym(args)}/network.xml",
