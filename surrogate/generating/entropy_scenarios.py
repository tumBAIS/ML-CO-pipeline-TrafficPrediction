...
def generate_entropy_network(dimension):
    nodes_locations_y = np.mgrid[0:dimension, 0:dimension][0, :, :]
    nodes_locations_x = np.mgrid[0:dimension, 0:dimension][1, :, :]
    nodes_indices = np.reshape(np.arange(dimension * dimension), (dimension, dimension))

    edges = []
    for idx_y, indices_x in enumerate(nodes_indices):
        for idx_x, node_index in enumerate(indices_x):
            # edge to top
            if (idx_y - 1) >= 0:
                edges.append((node_index, nodes_indices[idx_y - 1, idx_x]))
            # edge to right
            if (idx_x + 1) < dimension:
                edges.append((node_index, nodes_indices[idx_y, idx_x + 1]))
            # edge to bottom
            if (idx_y + 1) < dimension:
                edges.append((node_index, nodes_indices[idx_y + 1, idx_x]))
            # edge to left
            if (idx_x - 1) >= 0:
                edges.append((node_index, nodes_indices[idx_y, idx_x - 1]))

    nodes = pd.DataFrame({"x": nodes_locations_x.flatten(), "y": nodes_locations_y.flatten(), "id": nodes_indices.flatten()}).to_dict("records")
...
def generate_random_graph(args):
    if "street" in args.simulation:
        ...
        nodes, edges = generate_entropy_network(dimension=args.dimension_entropy)
        graph = {}
        graph["nodes_x"] = [node["x"] for node in nodes]
        graph["nodes_y"] = [node["y"] for node in nodes]
        graph["nodes_id"] = [node["id"] for node in nodes]
        graph["link_from"] = [edge[0] for edge in edges]
        graph["link_to"] = [edge[1] for edge in edges]
        graph["links_id"] = np.arange(len(graph["link_to"]))
        weights = [1] * len(graph["link_to"])
        g = Graph(graph, weights)
        g.draw()
...
def generate_random_population(args, G, minimum_start):
    ...
    max_loc = max(max(G["nodes_x"]), max(G["nodes_y"]))
    min_dist = 0.15 * max_loc

    if "high_entropy" in args.simulation:
        work_distribution_x = np.random.uniform(low=0, high=max_loc, size=args.population_size)
        work_distribution_y = np.random.uniform(low=0, high=max_loc, size=args.population_size)
        home_distribution_x = np.random.uniform(low=0, high=max_loc, size=args.population_size)
        home_distribution_y = np.random.uniform(low=0, high=max_loc, size=args.population_size)
    elif "low_entropy" in args.simulation:
        work_distribution_x = np.random.uniform(low=0, high=min_dist, size=args.population_size)
        work_distribution_y = np.random.uniform(low=0, high=min_dist, size=args.population_size)
        home_distribution_x = np.random.uniform(low=max_loc-min_dist, high=max_loc, size=args.population_size)
        home_distribution_y = np.random.uniform(low=max_loc-min_dist, high=max_loc, size=args.population_size)
    else:
        raise Exception("Wrong simulation name defined")
    work_distribution = [work_distribution_x, work_distribution_y]
    home_distribution = [home_distribution_x, home_distribution_y]
...
def save_random_scenario(args, graph, population, minimum_start, maximum_end):
    ...
    scenario["link_capacity"] = np.array([1000.0] * len(scenario["links_id"]))
...
if __name__ == '__main__':
    ...
    assert args.simulation in ["low_entropy", "high_entropy", "low_entropy_street", "high_entropy_street"]
