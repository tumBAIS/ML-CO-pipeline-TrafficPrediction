...
def run_simulation(args, scenario):
    ...
    thetas1 = [random.uniform(1, 100) for _ in range(len(scenario["links_id"]))]
    thetas2 = [random.uniform(1, 20) for _ in range(len(scenario["links_id"]))]

    thetas = np.array([np.array(thetas1), np.array(thetas2)])
